...
DATE_MAP = {
    'fraud_1.csv':  '2026-01-01',
    'fraud_2.csv':  '2026-01-02',
    'fraud_3.csv':  '2026-01-03',
    'normal_1.csv': '2026-01-04',
    'normal_2.csv': '2026-01-05',
}

print(f"\n  Creating synthetic datetime (date + time)...")
print(f"  Date assignment per source file:")
for fname, date in DATE_MAP.items():
    print(f"    {fname} → {date}")

# Combine synthetic date + actual time into a full datetime
master_df['synthetic_date'] = master_df['source_file'].map(DATE_MAP)
master_df['datetime'] = pd.to_datetime(
    master_df['synthetic_date'] + ' ' + master_df['timestamp'].astype(str),
    format='%Y-%m-%d %H:%M:%S',
    errors='coerce'
)
...
master_df.drop(columns=['synthetic_date'], inplace=True)
